...
export class BookingsService {
  ...
  async create(studentId: number, createDto: CreateBookingDto): Promise<Booking> {
    ...
    this.eventsService.notifyNewBooking(listing.tutor_id, fullBooking);
  ...
  async createRecurring(studentId: number, createDto: CreateRecurringBookingDto): Promise<Booking[]> {
    ...
    // 👈 УВЕДОМЛЕНИЕ О НОВОЙ СЕРИИ ЗАЯВОК (только первая заявка)
    if (createdBookings.length > 0) {
      const firstBooking = await this.findOne(createdBookings[0].id);
      this.eventsService.notifyNewBooking(listing.tutor_id, firstBooking);
    }
  ...
  async confirm(id: number, tutorId: number): Promise<Booking> {
    ...
    const fullBooking = await this.findOne(savedBooking.id);
    this.eventsService.notifyBookingUpdated(fullBooking);
  ...
  async cancelRecurring(recurringId: string, userId: number, role: string): Promise<void> {
    ...
    for (const booking of bookings) {
      if (booking.date >= today && booking.status !== BookingStatus.COMPLETED) {
        ...
        // 👈 УВЕДОМЛЕНИЕ ОБ ОТМЕНЕ КАЖДОГО ЗАНЯТИЯ
